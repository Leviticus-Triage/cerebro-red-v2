...
def test_unused_strategies_prioritized(orchestrator, test_strategies):
    ...
    for i in range(1, 6):
        ...
        assert (
            "Unused-first" in reasoning or "unused" in reasoning.lower()
        ...
        rotation = orchestrator._strategy_rotation[experiment_id]
        assert (
            strategy in rotation["used"]
        ), f"Iteration {i}: Strategy {strategy.value} not marked as used"

    # All 5 strategies should be used in first 5 iterations
    assert len(used_in_first_5) == 5, f"Expected 5 unique strategies, got {len(used_in_first_5)}"
    assert used_in_first_5 == set(
        test_strategies
...
def test_performance_tracking_updates(orchestrator, test_strategies):
    ...
    orchestrator._update_strategy_performance(
        experiment_id=experiment_id, strategy=strategy, judge_score=3.5, success=False
    )
    ...
    orchestrator._update_strategy_performance(
        experiment_id=experiment_id, strategy=strategy, judge_score=8.0, success=True
    )
    ...
    orchestrator._update_strategy_performance(
        experiment_id=experiment_id, strategy=strategy, judge_score=7.5, success=True
    )
...
def test_performance_based_selection(orchestrator, test_strategies):
    ...
    assert (
        strategy == test_strategies[1]
    ), f"Expected {test_strategies[1].value} (best performer), got {strategy.value}"
    assert (
        "Performance-based" in reasoning or "performance" in reasoning.lower()
    ), f"Expected 'Performance-based' in reasoning, got: {reasoning}"
    assert remaining == 0, f"Expected 0 remaining (all used), got {remaining}"
...
def test_strategy_rotation_with_single_strategy(orchestrator):
    ...
    for i in range(1, 4):
        ...
        # Update performance
        orchestrator._update_strategy_performance(experiment_id, strategy, 5.0 + i, i % 2 == 0)
...
def test_performance_selection_without_judge_score(orchestrator, test_strategies):
    ...
    assert (
        "Round-robin" in reasoning or "round-robin" in reasoning.lower()
...
def test_strategy_performance_success_rate_calculation(orchestrator, test_strategies):
    ...
    strategy, _, _, _ = orchestrator._get_next_strategy(experiment_id, iteration=6, judge_score=7.0)
