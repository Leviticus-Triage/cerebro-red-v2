...
class ConnectionManager:
    ...
    async def connect(
        self, experiment_id: UUID, websocket: WebSocket, verbosity: Optional[int] = None
    ) -> None:
        """
    ...
    async def broadcast(self, experiment_id: UUID, message: dict, min_verbosity: int = 0) -> None:
        ...
        logger.info(
            f"[WS-BROADCAST] Attempting broadcast for exp {experiment_id}, type={message.get('type')}, min_verbosity={min_verbosity}"
        )
        ...
        for connection in self.active_connections[experiment_id]:
            try:
                ...
                logger.debug(
                    f"[WS-BROADCAST] Connection verbosity={conn_verbosity}, required={min_verbosity}"
                )
                ...
                if conn_verbosity >= min_verbosity:
                    if connection.client_state == WebSocketState.CONNECTED:
                        ...
                        sent_count += 1
                        logger.debug(
                            f"[WS-BROADCAST]  Sent message type={message.get('type')} to connection"
                        )
                    else:
                        dead_connections.add(connection)
                        logger.warning("[WS-BROADCAST] Connection not in CONNECTED state")
                else:
                    filtered_count += 1
                    logger.debug(
                        f"[WS-BROADCAST]  Filtered message (verbosity {conn_verbosity} < {min_verbosity})"
                    )
        ...
        logger.info(
            f"[WS-BROADCAST] Summary: sent={sent_count}, filtered={filtered_count}, dead={len(dead_connections)}"
        )
...
async def websocket_scan_progress(websocket: WebSocket, experiment_id: UUID):
    """
    ...
    logger.info("[WS-ENDPOINT] ========== NEW CONNECTION ==========")
    ...
    if verbosity_error_message:
        try:
            await websocket.send_json({"type": "error", "error_message": verbosity_error_message})
        except Exception:
            ...
            try:
                ...
                if data.startswith("set_verbosity:"):
                    try:
                        ...
                        await websocket.send_json(
                            {"type": "verbosity_updated", "verbosity": new_verbosity}
                        )
...
async def send_vulnerability_found(
    experiment_id: UUID, vulnerability_id: UUID, severity: str, iteration: int
) -> None:
    """
...
async def send_error(
    experiment_id: UUID, error_message: str, iteration: Optional[int] = None
) -> None:
    """
...
async def send_failure_analysis(experiment_id: UUID, failure_analysis: Dict[str, Any]) -> None:
    """
    ...
    await manager.broadcast(
        ...
        min_verbosity=0,
    )  # Level 0: Always show (critical info)
...
async def send_llm_request(
    ...
    logger.info(
        f"[WS-SEND] send_llm_request called for exp {experiment_id}, role={role}, verbosity_required=2"
    )
...
async def send_llm_response(
    ...
    # DEBUG: Log before broadcast
    logger.info(
        f"[WS-SEND] send_llm_response called for exp {experiment_id}, role={role}, verbosity_required=2"
    )
    ...
    # DEBUG: Log message content
    logger.info(
        f"[WS-SEND] Message: type={message['type']}, role={role}, response_len={len(response)}, latency={latency_ms}ms"
    )
...
async def send_judge_evaluation(
    ...
    # DEBUG: Log before broadcast
    logger.info(
        f"[WS-SEND] send_judge_evaluation called for exp {experiment_id}, iteration={iteration}, score={judge_score}, success={success}"
    )
    ...
    # DEBUG: Log message content
    logger.info(
        f"[WS-SEND] Message: type={message['type']}, score={judge_score}, success={success}, verbosity_required=2"
    )
...
async def send_attack_mutation(
    ...
    message = {
        ...
        "original_prompt": (
            original_prompt[:300] + "..." if len(original_prompt) > 300 else original_prompt
        ),
        "mutated_prompt": (
            mutated_prompt[:500] + "..." if len(mutated_prompt) > 500 else mutated_prompt
        ),
        "timestamp": datetime.utcnow().isoformat(),
    }
...
async def send_target_response(
    experiment_id: UUID, iteration: int, prompt: str, response: str, latency_ms: float
) -> None:
    """
...
async def send_iteration_start(
    experiment_id: UUID, iteration: int, total_iterations: int, strategy: str
) -> None:
    """
...
async def send_mutation_start(
    experiment_id: UUID, iteration: int, strategy: str, original_prompt: str
) -> None:
...
async def send_mutation_end(
    experiment_id: UUID, iteration: int, strategy: str, mutated_prompt: str, latency_ms: float
) -> None:
    ...
    await manager.broadcast(
        experiment_id,
        {
            ...
            "strategy": strategy,
            "mutated_prompt": (
                mutated_prompt[:200] + "..." if len(mutated_prompt) > 200 else mutated_prompt
            ),
            "latency_ms": latency_ms,
            "timestamp": datetime.utcnow().isoformat(),
        },
...
async def send_judge_start(
    experiment_id: UUID, iteration: int, original_prompt: str, target_response: str
) -> None:
    ...
    await manager.broadcast(
        experiment_id,
        {
            ...
            "iteration": iteration,
            "original_prompt": (
                original_prompt[:150] + "..." if len(original_prompt) > 150 else original_prompt
            ),
            "target_response": (
                target_response[:200] + "..." if len(target_response) > 200 else target_response
            ),
            "timestamp": datetime.utcnow().isoformat(),
        },
        min_verbosity=3,
...
async def send_strategy_fallback(
    experiment_id: UUID, iteration: int, intended_strategy: str, fallback_strategy: str, reason: str
) -> None:
    """
