...
async def get_experiment_results(experiment_id: UUID, session: AsyncSession = Depends(get_session)):
    """
...
async def get_experiment_summary(experiment_id: UUID, session: AsyncSession = Depends(get_session)):
    """
    ...
    if format == "json":
        ...
        try:
            ...
            info_style = ParagraphStyle(
                "InfoStyle", parent=styles["Normal"], fontSize=11, spaceAfter=12
            )
            ...
            story.append(
                Paragraph(
                    f"<b>Created:</b> {experiment.created_at.strftime('%Y-%m-%d %H:%M:%S')}",
                    info_style,
                )
            )
            ...
            successful_iterations = [it for it in iterations if it.success]
            success_rate = (
                len(successful_iterations) / total_iterations if total_iterations > 0 else 0.0
            )
            avg_score = (
                sum(it.judge_score for it in iterations) / total_iterations
            ...
            if iterations:
                ...
                for it in iterations[:50]:  # Limit to first 50 iterations for PDF
                    iter_data.append(
                        [
                            ...
                            (
                                it.strategy_used[:30] + "..."
                                if len(it.strategy_used) > 30
                                else it.strategy_used
                            ),
                            f"{it.judge_score:.2f}",
                            "" if it.success else "",
                        ]
                ...
                iter_table = Table(
                    iter_data, colWidths=[0.5 * inch, 3 * inch, 1.5 * inch, 1 * inch]
                )
                iter_table.setStyle(
                    TableStyle(
                        [
            ...
            if vulnerabilities:
                ...
                for v in vulnerabilities:
                    ...
                    story.append(
                        Paragraph(
                            f"Strategy: {v.attack_strategy} | Iteration: {v.iteration_number}",
                            desc_style,
                        )
                    )
                    if v.description:
                        story.append(
                            Paragraph(
                                v.description[:500] + ("..." if len(v.description) > 500 else ""),
                                desc_style,
                            )
                        )
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": f"attachment; filename=experiment_{experiment_id}.pdf"
                },
            )
        except ImportError:
            raise HTTPException(
...
async def get_iteration_details(
    experiment_id: UUID, iteration_id: UUID, session: AsyncSession = Depends(get_session)
):
    """
