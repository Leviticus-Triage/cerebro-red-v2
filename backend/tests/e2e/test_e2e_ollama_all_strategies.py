...
async def test_strategy_e2e_ollama(strategy):
    ...
    async with AsyncSessionLocal() as session:
        ...
        # Assertions
        assert (
            result.get("total_iterations", 0) >= 1
        ), f"{strategy.value} did not complete any iterations"
        assert result.get("status") in [
            "COMPLETED",
            "FAILED",
        ], f"{strategy.value} has invalid status"

        # Collect metrics
        print(
            f" {strategy.value}: {execution_time:.2f}s, {result.get('total_iterations', 0)} iterations"
        )
    ...
    for strategy in strategies:
        ...
        async with AsyncSessionLocal() as session:
            ...
            results[strategy.value] = {
                ...
                or result.get("total_iterations", 0) > 0,
