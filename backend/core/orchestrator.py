        ...
        for i in range(1, max_iterations + 1):
            ...
            if success:
                ...
                attack_iteration = AttackIteration(
                    ...
                    experiment_id=experiment_id,
                    iteration_number=i,
                    strategy_used=strategy,  # Actually executed strategy
                    intended_strategy=intended_strategy,  # NEW: Originally selected
                    strategy_fallback_occurred=fallback_occurred,  # NEW
                    fallback_reason=fallback_reason,  # NEW
                    original_prompt=p0,
                    mutated_prompt=p_i,
                    target_response=r_i,
                    judge_score=s_i,
...
"""
    ...
    def _update_strategy_performance(
        ...
        Args:
            ...
            success: Whether iteration was successful (score >= threshold)
        """
        if experiment_id not in self._strategy_rotation:
            return
        ...
        logger.info(
            f"[Strategy Performance] {strategy.value}: "
            f"attempts={perf['total_attempts']}, "
