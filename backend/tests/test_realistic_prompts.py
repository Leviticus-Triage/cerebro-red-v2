...
async def test_no_test_artifacts_in_prompts(mutator, strategy):
    ...
    mutation = await mutator.mutate(original_prompt=original_prompt, strategy=strategy, iteration=1)
...
async def test_realistic_roleplay_injection(mutator):
    ...
    mutation = await mutator.mutate(
        original_prompt=original_prompt, strategy=AttackStrategyType.ROLEPLAY_INJECTION, iteration=1
    )
...
def test_get_realistic_template_fallback(mutator):
    ...
    mutated, source, name = mutator._get_realistic_template(
        AttackStrategyType.ROLEPLAY_INJECTION, "test prompt", category="non_existent_category_xyz"
    )
