...
async def test_full_pair_loop_uses_all_selected_strategies(mock_async_session):
    ...
    # Mock dependencies
    mock_mutator = Mock()
    mock_mutator.mutate = AsyncMock(
        return_value=Mock(
            output_prompt="mutated", mutation_params={"latency_ms": 100, "tokens_used": 50}
        )
    )
    ...
    mock_judge.get_feedback_dict = Mock(
        return_value={
    ...
    mock_target_llm.complete = AsyncMock(
        return_value=Mock(content="Test response", latency_ms=200, tokens_used=100, model="test")
    )
...
async def test_all_44_strategies_logged_in_db(mock_async_session):
    ...
    with patch("core.orchestrator.AsyncSessionLocal", return_value=mock_async_session):
        ...
        print(f"\n{'='*80}")
        print("STRATEGY COVERAGE TEST (Phase 6)")
        print(f"{'='*80}")
        ...
        assert len(unique_strategies) == len(
            ALL_STRATEGIES
        ), f"Only {len(unique_strategies)}/44 strategies used"
        assert unique_strategies == set(
            ALL_STRATEGIES
...
async def test_strategy_rotation_respects_user_selection(mock_async_session):
    ...
    mock_judge.get_feedback_dict = Mock(
        return_value={"target_response": "Test", "judge_score": 5.0, "judge_reasoning": "Test"}
    )

    mock_target_llm = Mock()
    mock_target_llm.complete = AsyncMock(
        return_value=Mock(content="Test", latency_ms=200, tokens_used=100, model="test")
    )
