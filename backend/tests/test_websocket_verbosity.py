...
async def test_verbosity_filtering_level_0():
    ...
    await manager.broadcast(
        experiment_id, {"type": "error", "message": "Test error"}, min_verbosity=0
    )
    ...
    await manager.broadcast(
        experiment_id, {"type": "llm_request", "prompt": "test"}, min_verbosity=2
    )
...
async def test_dead_connection_cleanup():
    ...
    assert (
        experiment_id not in manager.active_connections
        or ws_mock not in manager.active_connections[experiment_id]
    )
