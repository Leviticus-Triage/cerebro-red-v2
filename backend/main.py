...
from api.exceptions import CerebroException, cerebro_exception_handler, generic_exception_handler
...
def configure_logging():
    ...
    logger.info(" Logging configured: Level=DEBUG, Flush=Forced, Format=Structured")
    logger.debug("[DEBUG-TEST] This is a DEBUG log - if you see this, DEBUG logging works!")
...
async def lifespan(app: FastAPI):
    ...
    logger.info(" CEREBRO-RED v2 Starting...")
    ...
    logger.info(" Payload Coverage Audit Results:")
    logger.info(f"   Total Strategies: {audit_result['total_strategies']}")
    logger.info(
        f"   Covered: {audit_result['covered_strategies']} ({audit_result['coverage_percent']:.1f}%)"
    )
    logger.info(
        f"   Well-Covered (>=3 templates): {audit_result['well_covered_strategies']} ({audit_result['well_covered_percent']:.1f}%)"
    )
    logger.info(f"   {audit_result['recommendation']}")

    if audit_result["missing_strategies"]:
        logger.warning(
            f" Missing templates for {len(audit_result['missing_strategies'])} strategies:"
        )
    ...
    if audit_result["under_covered_strategies"]:
        logger.warning(
            f" Under-covered (<3 templates) for {len(audit_result['under_covered_strategies'])} strategies:"
        )
    ...
    audit_logger.log_event(event_type="payload_coverage_audit", metadata=audit_result)
    ...
    if audit_result["coverage_percent"] < 50:
        ...
        raise RuntimeError(
            f"Insufficient payload coverage: {audit_result['coverage_percent']:.1f}%"
        )
    ...
    async def scheduled_template_update():
        ...
        update_logger.info(
            f"⏰ Next template update scheduled in {initial_wait/3600:.1f} hours (at 2 AM UTC)"
        )
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    update_logger.warning(
                        f" Scheduled template update had failures: {', '.join(failed_repos)}"
                    )
    ...
    if settings.template_update.auto_update:
        ...
        logger.info(
            "Automatic template updates disabled (set TEMPLATE_UPDATE_AUTO_UPDATE=true to enable)"
        )
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    response = JSONResponse(
        ...
        content={"detail": exc.detail, "status": exc.status_code, "path": str(request.url.path)},
...
app.include_router(
    vulnerabilities_router, prefix="/api/v1/vulnerabilities", tags=["vulnerabilities"]
)
...
async def reset_circuit_breaker_endpoint(provider: str):
    ...
    try:
        ...
        return api_response(
            {"message": f"Circuit breaker reset for {provider}", "provider": provider}
        )
    except KeyError:
