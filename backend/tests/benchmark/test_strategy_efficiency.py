...
def mock_llm_client():
    ...
    mock_settings = Mock()
    mock_settings.get_llm_config = Mock(
        return_value={
            "provider": "ollama",
...
def mutator(mock_llm_client):
    ...
    return PromptMutator(
        llm_client=mock_llm_client, audit_logger=audit_logger, experiment_id=experiment_id
    )
...
async def test_all_strategies_efficiency_summary(mutator):
    ...
    for strategy in ALL_STRATEGIES:
        ...
        try:
            # Comment 1: mutate returns single PromptMutation object
            mutation = await mutator.mutate(
                original_prompt="Test prompt", strategy=strategy, iteration=0, feedback=None
            )
    ...
    # Print summary
    print(f"\n{'='*80}")
    print("STRATEGY EFFICIENCY SUMMARY (Phase 6)")
    print(f"{'='*80}")
