...
async def test_mutator_sends_websocket_events_for_attacker_llm():
    ...
    mutator = PromptMutator(
        llm_client=mock_llm_client, audit_logger=mock_audit_logger, experiment_id=experiment_id
    )

    # Mock WebSocket functions
    with (
        patch("api.websocket.send_llm_request", new_callable=AsyncMock) as mock_request,
...
async def test_mutator_sends_error_event_on_llm_failure():
    ...
    # Mock WebSocket functions
    with (
        patch("api.websocket.send_llm_request", new_callable=AsyncMock),
        ...
        result = await mutator._mutate_rephrase_semantic(
            prompt="Test prompt", feedback=feedback, iteration=3
        )
