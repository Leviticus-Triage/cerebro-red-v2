...
class TestPayloadCoverageAudit:
    ...
    def test_audit_well_covered_threshold(self):
        ...
        assert well_covered >= 80.0, f"Well-covered {well_covered:.1f}% is below 80% threshold"
    ...
    def test_audit_no_missing_strategies(self):
        ...
        assert len(missing) == 0, f"Found {len(missing)} strategies with no templates: {missing}"
...
class TestMutatorTemplateTracking:
    ...
    async def test_mutator_logs_template_source(self):
        ...
        mutator = PromptMutator(
            llm_client=mock_llm_client, audit_logger=mock_audit_logger, experiment_id="test-exp-123"
        )
        ...
        assert mutation.mutation_params["template_source"] in [
            "payloads.json",
            "hardcoded",
            "fallback",
        ]
    ...
    async def test_mutator_warns_on_hardcoded_fallback(self):
        ...
        mutation = await mutator.mutate(
            original_prompt="test", strategy=AttackStrategyType.OBFUSCATION_BASE64, iteration=1
        )
        ...
        if mutation.mutation_params.get("template_source") == "hardcoded":
            ...
            calls = [
                call
                for call in mock_audit_logger.log_error.call_args_list
                if call[1].get("error_type") == "template_fallback"
                or (len(call[0]) > 0 and "template_fallback" in str(call))
            ]
...
def mock_mutator():
    ...
    mock_llm = AsyncMock()
    mock_logger = Mock()
    return PromptMutator(llm_client=mock_llm, audit_logger=mock_logger, experiment_id="test-exp")
