...
from core.database import ExperimentDB, get_session, ExperimentRepository
...
async def create_experiment(
    experiment_config: ExperimentConfig, session: AsyncSession = Depends(get_session)
):
    """
    ...
    try:
        ...
        if not experiment_config.initial_prompts or not all(
            p.strip() for p in experiment_config.initial_prompts
        ):
        ...
        logger.info(
            f"[{request_id}] Experiment created successfully: {db_experiment.experiment_id}"
        )
...
async def update_experiment(
    ...
    if status_update:
        if status_update not in [s.value for s in ExperimentStatus]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid status: {status_update}"
            )
...
async def repeat_experiment(
    ...
    background_tasks.add_task(_run_experiment_with_error_handling, new_config, orchestrator)
...
async def get_experiment_iterations(
    experiment_id: UUID, session: AsyncSession = Depends(get_session)
):
    """
            ...
            "iterations": [
                {
                    ...
                    "intended_strategy": (
                        it.intended_strategy if hasattr(it, "intended_strategy") else None
                    ),  # NEW
                    "strategy_fallback_occurred": getattr(
                        it, "strategy_fallback_occurred", False
                    ),  # NEW
                    ...
                    "attacker_feedback": (
                        it.attacker_feedback if hasattr(it, "attacker_feedback") else None
                    ),
                }
    ...
    return api_response(
        {
            ...
            "strategy_fallback_rate": (
                fallback_count / total_iterations if total_iterations > 0 else 0.0
            ),  # NEW
