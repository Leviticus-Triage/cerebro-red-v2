...
class AppSettings(BaseSettings):
    ...
    demo_mode: bool = Field(
        default=False, description="Enable demo mode (read-only with mock data)"
    )
...
class TelemetrySettings(BaseSettings):
    ...
    path: Path = Field(default=Path("./data/audit_logs"), description="Path to audit log directory")
...
class OpenAISettings(BaseSettings):
    ...
    model_attacker: str = Field(default="gpt-4-turbo-preview", description="Attacker model name")
    model_target: str = Field(default="gpt-3.5-turbo", description="Target model name")
    model_judge: str = Field(default="gpt-4-turbo-preview", description="Judge model name")
    timeout: int = Field(default=300, ge=1, description="Request timeout in seconds")
...
class CircuitBreakerSettings(BaseSettings):
    ...
    jitter_enabled: bool = Field(default=True, description="Enable randomized backoff jitter")
    max_jitter_ms: int = Field(default=1000, ge=0, description="Maximum jitter in milliseconds")
...
class SecuritySettings(BaseSettings):
    ...
    api_key: str = Field(
        default="your-secret-api-key-change-this", description="API key for authentication"
    )
...
class TemplateUpdateSettings(BaseSettings):
    ...
    auto_update: bool = Field(
        default=False, description="Enable automatic daily template updates (runs at 2 AM UTC)"
    )
    update_interval_hours: int = Field(
        default=24, ge=1, description="Hours between automatic updates (default: 24 = daily)"
    )
...
class ExperimentSettings(BaseSettings):
    ...
    default_batch_size: int = Field(default=10, ge=1, description="Default batch size")
    enable_mutation_history: bool = Field(
        default=True, description="Enable mutation history tracking"
    )
    enable_detailed_telemetry: bool = Field(default=True, description="Enable detailed telemetry")
    vulnerability_severity_levels: str = Field(
        default="low,medium,high,critical",
        description="Vulnerability severity levels (comma-separated)",
    )
    auto_export_results: bool = Field(default=True, description="Auto-export results")
    export_format: str = Field(
        default="json,csv,pdf", description="Export formats (comma-separated)"
    )
