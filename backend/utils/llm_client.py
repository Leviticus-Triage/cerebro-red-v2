        ...
        **kwargs,
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                last_error = e
                error_type = classify_error(e)  # TRANSIENT
                ...
                if attempt < self.retry_attempts - 1:
                    ...
                    delay = calculate_backoff_with_jitter(
                        base_delay=self.settings.pair_algorithm.backoff_base_delay,
                        attempt=attempt,
                        multiplier=self.settings.pair_algorithm.backoff_multiplier,
                        max_delay=self.settings.pair_algorithm.backoff_max_delay,
                        jitter_enabled=self.settings.circuit_breaker.jitter_enabled,
                        max_jitter_ms=self.settings.circuit_breaker.max_jitter_ms,
                    )

                    verbose_logger.log(
                        "WARNING",
                        f"⏳ Rate limit hit, retrying in {delay:.2f}s (attempt {attempt+1}/{self.retry_attempts})",
                        component="LLM",
                ...
                # Permanent error - don't retry, don't count toward circuit opening
                error_type = classify_error(e)  # PERMANENT
                circuit_breaker.record_failure(error_type)

                verbose_logger.log(
                    "ERROR",
                    " Authentication failed (permanent error, not retrying)",
                    component="LLM",
                ...
                last_error = e
                error_type = classify_error(e)  # Could be TRANSIENT or UNKNOWN
                ...
                # Unexpected errors - classify and record
