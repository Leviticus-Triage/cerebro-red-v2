...
def test_enum_count_matches():
    ...
    assert len(backend_values) == len(
        frontend_values
    ), f"Enum count mismatch: Backend={len(backend_values)}, Frontend={len(frontend_values)}"

    assert len(backend_values) == 44, f"Expected 44 strategies, got {len(backend_values)}"
...
def test_enum_values_match_exactly():
    ...
    if only_in_backend or only_in_frontend:
        ...
        pytest.fail(
            f"Enum mismatch: {len(only_in_backend)} backend-only, {len(only_in_frontend)} frontend-only"
        )
...
"""
    ...
    print("   Run: ./backend/scripts/check_enum_sync.sh")
...
def test_all_strategies_documented():
    ...
    if undocumented:
        ...
        print("     Warning: Some strategies are not documented")
    else:
