                ...
                prompts.append(
                    {
                        ...
                        "provider": (
                            md_path.stem.upper() if md_path.stem != "-MISCELLANEOUS-" else "GENERAL"
                        ),
                    }
                )
...
def categorize_l1b3rt4s_prompt(prompt_data: Dict[str, Any]) -> str:
    ...
    if (
        "godmode" in prompt_lower
        or ".-.-.-" in prompt_data["prompt"]
        or "|l|o|v|e|" in prompt_lower
    ):
...
if __name__ == "__main__":
    ...
    print(
        f"Found {sum(len(p) for p in prompts_by_category.values())} prompts in {len(prompts_by_category)} categories"
    )
