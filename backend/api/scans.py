...
# Global registry to track running experiment tasks
# This allows us to cancel/pause experiments by cancelling their asyncio tasks
_running_experiment_tasks: Dict[UUID, asyncio.Task] = {}
...
async def _run_experiment_with_error_handling(
    config: ExperimentConfig, orchestrator: RedTeamOrchestrator
):
    """
    ...
    # Task is already registered in start_scan before this function is called
    # No need to register again here

    try:
        logger.info(f"[DIAG-TASK] WRAPPER CALLED for {experiment_id}")
        logger.info(f"[DIAG-WRAPPER] Experiment: {config.name}")
        logger.info("[DIAG-WRAPPER] About to call orchestrator.run_experiment()")

        await orchestrator.run_experiment(config)
        logger.info(f"[DIAG-WRAPPER] run_experiment completed successfully for {experiment_id}")
    except asyncio.CancelledError:
        logger.warning(f"[TASK-REGISTRY] Experiment {experiment_id} task was cancelled")
        # Update status to FAILED when cancelled
        async with AsyncSessionLocal() as error_session:
            error_repo = ExperimentRepository(error_session)
            await error_repo.update_status(experiment_id, ExperimentStatus.FAILED.value)
            await error_session.commit()
        raise  # Re-raise to properly handle cancellation
    except Exception as e:
        logger.error("[DIAG-WRAPPER] ========== EXPERIMENT FAILED ==========")
        ...
        logger.error("[DIAG-WRAPPER] Full Traceback:")
        logger.error(f"\n{traceback.format_exc()}")
        logger.error("[DIAG-WRAPPER] ========================================")
        ...
        _running_experiment_tasks.pop(experiment_id, None)
        logger.info(f"[TASK-REGISTRY] Unregistered task for experiment {experiment_id}")


async def _run_batch_experiments_concurrently(
    experiments: List[ExperimentConfig], orchestrator: RedTeamOrchestrator
):
    """
...
async def start_scan(
    ...
    if experiment_config.strategies:
        ...
        if experiment_config.strategies and isinstance(experiment_config.strategies[0], str):
            logger.warning(
                "[start_scan] Strategies are strings, converting to AttackStrategyType enums"
            )
            experiment_config.strategies = [
                AttackStrategyType(s) for s in experiment_config.strategies
            ]
            logger.info(
                f"[start_scan] Converted strategies: {[s.value for s in experiment_config.strategies]}"
            )
        elif experiment_config.strategies and isinstance(
            experiment_config.strategies[0], AttackStrategyType
        ):
            logger.info("[start_scan] Strategies are already AttackStrategyType enums")
        logger.info(
            f"[start_scan] First strategy type: {type(experiment_config.strategies[0]) if experiment_config.strategies else 'empty'}"
        )
    ...
    logger.info("[DIAG] ========== START_SCAN ENDPOINT CALLED ==========")
    ...
    try:
        ...
        logger.info(
            f"[DIAG] Strategies: {[s.value if hasattr(s, 'value') else str(s) for s in experiment_config.strategies]}"
        )
    except Exception as log_err:
        logger.error(
            f"[DIAG] Error logging experiment config: {type(log_err).__name__}: {str(log_err)}"
        )
        import traceback

        logger.error(f"[DIAG] Traceback: {traceback.format_exc()}")

    # CRITICAL FIX: Use asyncio.create_task instead of BackgroundTasks
    # This allows us to track the task in _running_experiment_tasks for pause/cancel/delete
    import asyncio

    loop = asyncio.get_event_loop()
    logger.info(f"[DIAG-START] Creating asyncio.Task for experiment {experiment_id}")
    logger.info(f"[DIAG-START] Orchestrator type: {type(orchestrator)}")
    logger.info(f"[DIAG] Event loop running: {loop.is_running()}")

    # Create task and register it immediately
    task = asyncio.create_task(_run_experiment_with_error_handling(experiment_config, orchestrator))
    _running_experiment_tasks[experiment_id] = task
    logger.info(f"[TASK-REGISTRY] Registered task for experiment {experiment_id}")
...
async def pause_scan(
    ...
    # Pause experiment (sets status flag that will be checked in loop)
    await orchestrator.pause_experiment(experiment_id)

    # CRITICAL FIX: Cancel the running task to immediately stop execution
    task = _running_experiment_tasks.get(experiment_id)
    if task and not task.done():
        logger.info(f"[PAUSE] Cancelling task for experiment {experiment_id}")
        task.cancel()
        try:
            await task
        except asyncio.CancelledError:
            logger.info(f"[PAUSE] Task for experiment {experiment_id} cancelled successfully")
        except Exception as e:
            logger.error(f"[PAUSE] Error during task cancellation for {experiment_id}: {e}")
        _running_experiment_tasks.pop(experiment_id, None)
        logger.info(f"[PAUSE] Removed experiment {experiment_id} from running tasks registry")
...
async def resume_scan(
    ...
    return api_response(
        {"experiment_id": experiment_id, "status": "running", "message": "Experiment resumed"}
    )
...
async def cancel_scan(
    ...
    # Cancel experiment (sets status flag)
    await orchestrator.cancel_experiment(experiment_id)

    # CRITICAL FIX: Cancel the running task immediately
    task = _running_experiment_tasks.get(experiment_id)
    if task and not task.done():
        logger.info(f"[CANCEL] Cancelling task for experiment {experiment_id}")
        task.cancel()
        try:
            await task
        except asyncio.CancelledError:
            logger.info(f"[CANCEL] Task for experiment {experiment_id} cancelled successfully")
        except Exception as e:
            logger.error(f"[CANCEL] Error during task cancellation for {experiment_id}: {e}")
        _running_experiment_tasks.pop(experiment_id, None)
        logger.info(f"[CANCEL] Removed experiment {experiment_id} from running tasks registry")
...
async def get_scan_status(
    ...
    print(
        f"[DEBUG] Progress: num_prompts={num_prompts}, max_iter={experiment.max_iterations}, total={total_iterations}, current={current_iteration}"
    )
...
async def start_batch_scan(
    ...
    background_tasks.add_task(
        _run_batch_experiments_concurrently, request.experiments, orchestrator
    )

    logger.info(
        f"[DIAG-BATCH] All {len(experiment_ids)} experiments dispatched for concurrent execution"
    )
