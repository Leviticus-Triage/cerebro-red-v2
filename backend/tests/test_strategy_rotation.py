...
def test_strategy_rotation_all_strategies_used(orchestrator):
    ...
    assert len(used_strategies) == len(
        strategies
    ...
    for expected_strategy in strategies:
        assert (
            expected_strategy in used_strategies
...
def test_forced_rotation_every_5th_iteration(orchestrator):
    ...
    for i in range(1, 5):
        ...
        assert (
            "Intelligent selection" in reasoning
            or "Fallback" in reasoning
            or "Round-robin" in reasoning
        )

    # Iteration 5: Forced rotation
    strategy_5, reasoning_5, _, _ = orchestrator._get_next_strategy(
        experiment_id, iteration=5, judge_score=2.0, target_response="I can't help with that."
    )
    ...
    strategy_10, reasoning_10, _, _ = orchestrator._get_next_strategy(
        experiment_id, iteration=10, judge_score=2.0, target_response="I can't help with that."
    )
