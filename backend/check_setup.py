...
def check_dependencies():
    ...
    if missing:
        ...
        print("   Lösung: pip3 install -r requirements.txt")
        return False
...
def check_env_file():
    ...
    if not env_path.exists():
        ...
        print("   Lösung: cp ../.env.example ../.env")
        return False
    ...
    with open(env_path) as f:
        ...
        if "DEFAULT_LLM_PROVIDER" not in content:
            print("  .env scheint nicht konfiguriert zu sein")
            print("   Lösung: nano ../.env")
            return False

    print(" .env scheint konfiguriert zu sein")
    return True


def check_directories():
    """Prüft ob notwendige Verzeichnisse existieren."""
    required_dirs = ["../data/experiments", "../data/audit_logs", "../data/results"]
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if "qwen3:8b" in result.stdout:
                print(" qwen3:8b verfügbar")
            else:
                print("  qwen3:8b nicht gefunden")
                print("   Lösung: ollama pull qwen3:8b")

            if "qwen3:14b" in result.stdout:
                print(" qwen3:14b verfügbar")
            else:
                print("  qwen3:14b nicht gefunden")
                print("   Lösung: ollama pull qwen3:14b")

            return True
        else:
            print(" Ollama nicht erreichbar")
            print("   Lösung: ollama serve (in neuem Terminal)")
            return False
    except FileNotFoundError:
        print(" Ollama nicht installiert")
        print("   Lösung: https://ollama.ai/download")
        return False
...
def check_database():
    ...
    if db_path.exists():
        ...
        print("   Lösung: alembic upgrade head")
        return False
...
def check_imports():
    ...
    try:
        from core.models import ExperimentConfig  # noqa: F401

        print(" core.models importierbar")
    except ImportError as e:
        print(f" core.models nicht importierbar: {e}")
        return False

    try:
        from utils.config import get_settings  # noqa: F401

        print(" utils.config importierbar")
    except ImportError as e:
