      ...
      <Table className="w-full">
        ...
        <TableBody>
          {logs.map((log) => {
            const isExpanded = expandedRows.has(log.id);
            const roleColor = getRoleBadgeColor(log.role);
            // Extract content - ensure it's always a string
            let contentText: string = '';
            const rawContent = log.metadata?.response || log.metadata?.prompt || log.content;
            if (typeof rawContent === 'object') {
              const jsonStr = JSON.stringify(rawContent);
              // If it's just an empty object, show a more meaningful message
              if (jsonStr === '{}' || jsonStr === '[]') {
                contentText = typeof log.content === 'string' ? log.content : `${log.type} event`;
              } else {
                contentText = jsonStr;
              }
        ...
        <TableBody>
          {logs.map((log) => {
            ...
            if (typeof rawContent === 'object') {
              const jsonStr = JSON.stringify(rawContent);
              if (jsonStr === '{}' || jsonStr === '[]') {
                errorContent = `${log.type} error occurred`;
              } else {
                errorContent = jsonStr;
              }
            } else if (typeof rawContent === 'string') {
              errorContent = rawContent;
            } else {
              errorContent = String(rawContent || `${log.type} error`);
            }
...
export const LiveLogPanel: React.FC<LiveLogPanelProps> = ({
  ...
  console.log('[LIVELOG]   - Responses:', responseLogs.length);
  console.log('[LIVELOG]   - Judge:', judgeLogs.length);
  console.log('[LIVELOG]   - Errors:', errorLogs.length);
  console.log('[LIVELOG]   - All:', allLogs.length);
