    ...
    async def broadcast(self, experiment_id: UUID, message: dict, min_verbosity: int = 0) -> None:
        ...
        for connection in self.active_connections[experiment_id]:
            try:
                ...
                # Debug logging for verbosity filtering
                import logging
                logger = logging.getLogger(__name__)
                if logger.isEnabledFor(logging.DEBUG):
                    if conn_verbosity >= min_verbosity:
                        logger.debug(
                            f"WS Broadcast: {message.get('type', 'unknown')} to connection "
                            f"(verbosity {conn_verbosity} >= {min_verbosity})"
                        )
                    else:
                        logger.debug(
                            f"WS Filtered: {message.get('type', 'unknown')} for connection "
                            f"(verbosity {conn_verbosity} < {min_verbosity})"
                        )
