...
def add_missing_categories():
    ...
    backup_file = (
        data_dir / f"payloads.json.backup.{datetime.now().strftime('%Y%m%d_%H%M%S')}_missing"
    )
    ...
    missing_categories = {
        ...
        "context_flooding": [
            ...
            },
            {
                ...
                + "Step 1: Read. Step 2: Understand. " * 40
                + "\n{original_prompt}",
    ...
    for category, templates in missing_categories.items():
        if category not in payloads["categories"]:
            ...
            print(
                f"     Skipping {category} (already has {len(payloads['categories'][category])} templates)"
            )
