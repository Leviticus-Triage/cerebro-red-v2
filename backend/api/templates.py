    ...
    try:
        if repository:
            ...
            if not result.get("success"):
                ...
                failed_repos = [
                    r["repo_name"] for r in result["repositories"] if not r.get("success")
                ]
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def list_template_repositories():
    ...
    try:
        ...
        return api_response({"repositories": repositories, "total": len(repositories)})
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_template_repository(repo_name: str):
    ...
    try:
        ...
        if not repo:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Repository '{repo_name}' not found"
            )
...
async def get_repository_update_history(repo_name: str, limit: int = Query(100, ge=1, le=1000)):
    """
    ...
    try:
        ...
        return api_response({"repository": repo_name, "history": history, "total": len(history)})
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def list_jailbreak_templates(
    ...
    try:
        ...
        if not data_path.exists():
            return api_response({"categories": {}, "total_templates": 0, "total_categories": 0})
        ...
        for cat_name, cat_data in payloads_data.items():
            ...
            if search:
                ...
                filtered_templates = [
                    ...
                    or search_lower in cat_data.get("description", "").lower()
                ]
...
async def list_jailbreak_backups():
    ...
    try:
        ...
        return api_response({"backups": backups, "total": len(backups)})
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def import_jailbreak_templates(
    ...
    try:
        ...
        total_imported = sum(
            ...
            for cat in final_data.values()
            if isinstance(cat, dict) and "templates" in cat
        )
        ...
        return api_response(
            {
                ...
                "total_categories": len(
                    [k for k in final_data.keys() if k not in ["version", "source", "last_updated"]]
                ),
...
async def export_jailbreak_templates(
    ...
    try:
        ...
        if not data_path.exists():
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="advanced_payloads.json not found"
            )
...
async def add_jailbreak_template(
    category: str, template: str = Query(..., description="Template string to add")
):
    """
...
async def delete_jailbreak_template(category: str, template_index: int):
    """
...
async def create_template(
    template: ExperimentTemplateCreate, session: AsyncSession = Depends(get_session)
):
    """
...
async def list_templates(
    ...
    templates, total = await repo.list_all(
        limit=page_size, offset=offset, is_public=is_public, created_by=created_by, tags=tag_list
    )

    response = ExperimentTemplateListResponse(
        items=[db_to_response(t) for t in templates], total=total, page=page, page_size=page_size
    )
...
@router.delete(
    "/{template_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(verify_api_key)]
)
async def delete_template(template_id: UUID, session: AsyncSession = Depends(get_session)):
    """
    Delete template.
    """
    repo = ExperimentTemplateRepository(session)
...
async def use_template(template_id: UUID, session: AsyncSession = Depends(get_session)):
    """
