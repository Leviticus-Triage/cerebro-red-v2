...
@pytest.mark.skip(
    reason="Requires Ollama running - use with OLLAMA_BASE_URL=http://localhost:11434"
)
async def test_latency_benchmark():
    ...
    mutator = PromptMutator(llm_client=llm_client, audit_logger=audit_logger, experiment_id=uuid4())
    ...
    for i in range(num_samples):
        ...
        target_response = await llm_client.complete(
            [{"role": "user", "content": mutation.output_prompt}], role="target"
        )
    ...
    for name, latencies in [
        ...
        if latencies:
            ...
            print(
                f"{name:<15} {mean_latency:<12.2f} {std_latency:<12.2f} "
