...
async def get_vulnerability_statistics(session: AsyncSession = Depends(get_session)):
    """
            ...
            "vulnerabilities": [
                {
                    ...
                    # IMPROVED: Ensure attack_strategy is always a string
                    "attack_strategy": (
                        v.attack_strategy
                        if isinstance(v.attack_strategy, str)
                        else v.attack_strategy.value
                    ),
...
async def get_vulnerability(vulnerability_id: UUID, session: AsyncSession = Depends(get_session)):
    """
    ...
    return api_response(
        {
            ...
            # IMPROVED: Ensure attack_strategy is always a string
            "attack_strategy": (
                vulnerability.attack_strategy
                if isinstance(vulnerability.attack_strategy, str)
                else vulnerability.attack_strategy.value
            ),
            ...
            "discovered_at": (
                vulnerability.discovered_at.isoformat() if vulnerability.discovered_at else None
            ),
